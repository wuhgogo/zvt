...
class JoinquantChinaBlockStockRecorder(TimeSeriesDataRecorder):
    ...
    def record(self, entity, start, end, size, timestamps):
        df_block_stock = request_jqdata('get_concept_stocks', concept_code=entity.code)
        the_list = []
        for _, stock in df_block_stock.iterrows():
            stock_id = to_entity_id(stock, 'stock')
            stock_entity = Stock.query_data(provider=self.provider, entity_id=stock_id)
...
if __name__ == '__main__':
    ...
    # recorder = JoinquantChinaBlockRecorder(codes=['new_cbzz'])
    # recorder.run()
    # JoinquantChinaBlockRecorder().run()
    JoinquantChinaBlockStockRecorder(codes=['GN705']).run()
