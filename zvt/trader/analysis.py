...
from contract.normal_data import NormalData
from domain import Index1dKdata
...
def analysis(data_df=None):
    trader_result = data_df.copy()[['trader_name', 'timestamp', 'all_value']]
    ...
    annualized_income = pow(trader_result.ix[len(trader_result.index) - 1, 'all_value'] / data_df.ix[0,
                                                                                                     'all_value'],
                            365.0 /
                            trader_days) - 1
    ...
    max_losses_times = trader_result[trader_result['profit_losses'] == -1].sort_values(by='profit_losses_list',
                                                                                       ascending=False)[
        'profit_losses_list'].iloc[0]
    return RiskAnalysis(accumulated_net=accumulated_net,
...
def value_pct(data_df):
    pct_change = data_df.copy()[['name', 'timestamp', 'bench_value']]
    pct_change['value_pct'] = pct_change['bench_value'].pct_change()
    pct_change['value_pct'].fillna(value=0, inplace=True)
    pct_change['value'] = (1.0 + pct_change['value_pct']).cumprod()
    pct_change.drop(['bench_value', 'value_pct'], axis=1, inplace=True)
    return pct_change


def draw(data_df, bench_entity_id):
    start_time = data_df['timestamp'].head(1).iloc[0]
    end_time = data_df['timestamp'].tail(1).iloc[0]
    bench_data = value_pct(Index1dKdata.query_data(provider='joinquant', start_timestamp=start_time,
                                                   end_timestamp=end_time,
                                                   entity_id=bench_entity_id).rename(columns={'close': 'bench_value'}))
    trader_data = value_pct(data_df.rename(columns={'trader_name': 'name', 'all_value': 'bench_value'}))
    import plotly.io as pio
    pio.renderers.default = "browser"
    drawer = Drawer(main_data=NormalData(trader_data.append(bench_data).reset_index(drop=True),
                                         category_field='name'))
...
class RiskAnalysisTradingListener(TradingListener):
    ...
    def __init__(self, trader_name, draw_result=False, bench_entity_id='index_sh_000300') -> None:
        self.trader_name = trader_name
        self.draw_result = draw_result
        self.bench_entity_id = bench_entity_id

    def on_trading_finish(self, timestamp):
        reader = AccountStatsReader(trader_names=[self.trader_name])
        df = reader.data_df
        risk_analysis = analysis(df)
        print(risk_analysis)
        if self.draw_result:
            draw(df, self.bench_entity_id)
